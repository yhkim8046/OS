...
scheduler(void)
{
  ...
  for(;;){
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->state == RUNNABLE) {
        if (!highest_priority_proc || p->nice < highest_priority_proc->nice) {
          //unlock the previosuly selected process
          if (highest_priority_proc)
            release(&highest_priority_proc->lock);
          // select a new process and maintain the lock
    ...
    // implement the highest priority process
    if (highest_priority_proc) {
